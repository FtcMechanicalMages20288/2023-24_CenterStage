...
public class Robot {
    ...
    private DcMotor lm;     //  control the left front drive wheel
    private DcMotor rm;    //  control the right front drive wheel
    private DcMotor blm;      //  control the left back drive wheel
    private DcMotor brm;     //  control the right back drive wheel

    private DcMotor Intake;      //  the Lateral (left/right) Odometry Module (may overlap with motor, or may not)
    private DcMotor LeadScrew;       //  the Axial (front/back) Odometry Module (may overlap with motor, or may not)
    ...
    public boolean readSensors() {
        rawDriveOdometer = LeadScrew.getCurrentPosition() * (INVERT_DRIVE_ODOMETRY ? -1 : 1);
        rawStrafeOdometer = Intake.getCurrentPosition() * (INVERT_STRAFE_ODOMETRY ? -1 : 1);
